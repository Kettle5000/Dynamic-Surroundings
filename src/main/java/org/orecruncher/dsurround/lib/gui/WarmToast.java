...
public class WarmToast  implements Toast {
    private static final Profile DEFAULT_PROFILE = new Profile(new ResourceLocation("minecraft", "textures/gui/toasts.png"), 5000, ColorPalette.GOLD, ColorPalette.WHITE);
    ...
    public @NotNull Toast.Visibility render(@NotNull GuiGraphics guiGraphics, @NotNull ToastComponent toastComponent, long lastChanged) {
        ...
        if (i == 160 && this.messageLines.size() <= 1) {
            guiGraphics.blit(this.profile.sprite, 0, 0, 0, 0, i, this.height(), 256, 32);
        } else {
    ...
    private void renderBackgroundRow(GuiGraphics guiGraphics, int i, int j, int k, int l) {
        ...
        guiGraphics.blit(this.profile.sprite, 0, k, 0, j, m, l, 256, 32);

        for(int o = m; o < i - n; o += 64) {
            guiGraphics.blit(this.profile.sprite, o, k, 32, j, Math.min(64, i - o - n), l, 256, 32);
        }

        guiGraphics.blit(this.profile.sprite, i - n, k, 160 - n, j, n, l, 256, 32);
    }
